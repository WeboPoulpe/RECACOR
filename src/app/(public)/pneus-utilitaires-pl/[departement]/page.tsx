...
const DEPARTMENTS = {
  ardennes: { label: "Ardennes", code: "08" },
  "haute-marne": { label: "Haute-Marne", code: "52" },
  "meurthe-et-moselle": { label: "Meurthe-et-Moselle", code: "54" },
  meuse: { label: "Meuse", code: "55" },
  vosges: { label: "Vosges", code: "88" },
} as const;

type DepartmentSlug = keyof typeof DEPARTMENTS;

export function generateStaticParams() {
  return Object.keys(DEPARTMENTS).map((departement) => ({ departement }));
}

function getDepartment(slug: string) {
  if (!(slug in DEPARTMENTS)) return null;
...
export async function generateMetadata({
  ...
  const department = getDepartment(departement);
  if (!department) return {};

  return {
    title: `Pneus poids lourd en ${department.label} | Devis transport et remorque`,
    description: `Besoin de pneus poids lourd en ${department.label} ? Recacor étudie votre dimension, votre véhicule et votre usage pour préparer un devis professionnel.`,
    alternates: { canonical: `/pneus-utilitaires-pl/${departement}` },
    openGraph: {
      title: `Pneus poids lourd en ${department.label} | Recacor`,
      description: `Pneus PL, transport, remorque et besoins TP/BTP : transmettez votre dimension et votre localisation pour préparer une demande professionnelle.`,
      url: `https://www.recacor.fr/pneus-utilitaires-pl/${departement}`,
      siteName: "Recacor",
      locale: "fr_FR",
      type: "website",
    },
...
export default async function PneusPoidsLourdDepartmentPage({
  params,
}: {
  params: Promise<{ departement: string }>;
}) {
  ...
  const faqs = [
    {
      q: `Que faut-il indiquer pour un devis pneus PL en ${department.label} ?`,
    ...
    {
      q: `Quels besoins pneus PL puis-je transmettre depuis la ${department.label} ?`,
      a: "Les demandes de transport, remorque, TP/BTP et flotte professionnelle peuvent être étudiées selon la dimension, l'usage et la localisation indiqués dans le formulaire.",
    },
    {
  ...
  const canonical = `https://www.recacor.fr/pneus-utilitaires-pl/${departement}`;
    ...
    <>
      <BreadcrumbJsonLd
        items={[
          ...
          { name: `Pneus poids lourd en ${department.label}`, url: canonical },
        ]}
      />
      <FaqJsonLd items={faqs} id={`pneus-pl-${departement}`} />
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl">
            <p className="inline-flex items-center gap-2 border-l-4 border-yellow-400 pl-3 text-xs font-black uppercase tracking-[0.16em] text-white/75">
              <MapPin className="h-4 w-4 text-yellow-400" /> {department.label} · professionnels
            </p>
            <h1 className="mt-6 max-w-4xl font-heading text-5xl font-black uppercase leading-[0.9] sm:text-6xl lg:text-8xl">
              Pneus poids lourd
              <span className="block text-yellow-400">en {department.label}</span>
            </h1>
        ...
        <div className="mx-auto grid max-w-6xl gap-8 px-4 sm:px-6 lg:grid-cols-[1fr_0.8fr] lg:px-8">
          <div>
            ...
            <div className="mt-8 rounded-[4px] border border-blue-100 bg-blue-50 p-5 text-sm leading-6 text-slate-700">
              <p className="font-black text-[var(--recacor-night)]">Vous ne connaissez pas la dimension ?</p>
              <p className="mt-1">Indiquez le modèle du véhicule, l&apos;engin concerné, la commune du besoin et ce que vous pouvez lire sur le pneu. Une photo du marquage peut aussi aider l&apos;équipe à reprendre la bonne dimension.</p>
            </div>
          </div>
          <aside className="rounded-[4px] bg-[var(--recacor-night)] p-7 text-white">
            <Truck className="h-10 w-10 text-yellow-400" />
            <h2 className="mt-5 font-heading text-4xl font-black uppercase leading-none">{department.label}</h2>
            <p className="mt-4 text-sm leading-6 text-white/68">
              Pour une demande dans le département {department.code}, précisez la commune, le véhicule et l&apos;usage dans le formulaire. La qualification commerciale se fait à partir des informations réellement transmises.
            </p>
